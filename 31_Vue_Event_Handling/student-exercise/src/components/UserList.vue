  ...
  <div class="container">
    <table id="tblUsers">
      ...
      <tbody>
        <tr>
          <td>
            <input type="checkbox" id="selectAll" v-model="selected" @change="allSelected" />
          </td>
          <td>
    ...
    <form v-show="showForm"  id="frmAddNewUser">
      ...
      <button type="submit" v-on:click.prevent="saveUser"  class="btn save">Save User</button>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    flipStatus(id){ 
     ...
     deleteSelectedUsers(){
      let deleteUs = this.users;
      for(let i = 0; i < this.selectedUserIDs.length; i ++){
      deleteUs = deleteUs.filter(user=> {
